...
function CodeMatrix() {
    ...
    const renderCards = (data) => data && data.map((item) => <CodeCard key={item} {...parse(item)} />);
        ...
        <>
            {dataSource && dataSource.length ? (
                <div style={{ columnCount: Cols(listType).cc, columnGap: '16px' }}>
                    {keyword ? (searchResults.length ? renderCards(searchResults) : null) : renderCards(dataSource)}
                </div>
            ) : (
                <CodeEmpty />
            )}
            {keyword && !searchResults.length && <Empty title="暂未找到匹配的筛选结果" description={`"${keyword}"`} />}
        </>
    );
}
